...
`runner.py` のフィルタ(買取・売値いずれかが50万円以上)基準では
**176件**(買取側112件+販売側64件)が残る。ただし晴れる屋2は1レコードに
買取・売値を両方持つため、純粋に「販売価格(sell_price)が50万円以上」の
観点で数えると**107件**になる(晴れる屋2のsell分43件・福福トレカ
コレクション29件・カードラッシュ19件・フルアヘッド13件・CBトレコロ3件)。

- 買取(buy)で実データ抽出できているショップ(件数順、カッコ内は50万円
  以上の件数):
...
- 販売(sell)で実データ抽出できているショップ(通販サイト、件数順。カッコ
  内は50万円以上の件数):
  - トレカプラザ55 720件(0件) — 新規発見。タイプ別ページ12種を巡回
  - フルアヘッド 500件(13件) — 新規発見。PSA10鑑定品専門コーナー全10
    ページを巡回。最高548万円と販売側で最も高額カードの収穫が多い
  - カードラッシュ 84件(19件)
  - 福福トレカコレクション 80件(29件) — 新規発見。既存の福福トレカとは
    別ドメイン。ページネーション全7ページを巡回。最高228万円
  - トレカキャンプ 65件(0件)
  - 福福トレカ 64件(0件)
  - カードラボ 48件(0件)
  - Japan-toreca 42件(0件) — 新規発見。PSA鑑定品専門。最高42万円と
    しきい値に迫るが届かず
