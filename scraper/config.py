...
SHOPS: List[ShopConfig] = [
    ...
    ShopConfig(
        ...
        # ドラゴン/無色)とセール2種のgidを列挙して回った結果、1ページ
        # あたり60件・全12ページで合計720件抽出できることを確認済み
        # (2026-09-04)。専用ページ内はポケモンカードのみのため
        # [ポケモンカードゲーム]表記フィルタは不要と判明し撤廃した。
        buy_url=None,
        sell_url=None,
        sell_urls=[
    ...
    ShopConfig(
        ...
        # (?mode=cate&cbid=2929264&csid=0)で1ページ12件・全7ページの
        # ページネーションを持ち、page=1〜7を回って全78件抽出できることを
        # 確認済み(div.c-items配下の`a[href^="?pid="]`単位)。
        buy_url=None,
        sell_url=None,
        sell_urls=[
    ...
    ShopConfig(
        ...
        # 浅く直接対応付けできないため出現順でzipして対応)。全10ページ
    ...
    ShopConfig(
        ...
        # 構造化されており、2ページ合計42件・最高42万円を確認済み
        # (2026-09-04)。惜しくも50万円のしきい値には届かなかった。
        buy_url=None,
        sell_url=None,
        sell_urls=[
