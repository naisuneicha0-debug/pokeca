...
def run() -> None:
    ...
    for shop in SHOPS:
        ...
        for direction, url in (("buy", shop.buy_url), ("sell", shop.sell_url)):
            ...
            try:
                ...
                shop_result["errors"].append(f"{direction}取得失敗: {e}")
                print(f"[ERROR] {shop.shop_name} ({direction}) fetch: {e}")
                continue
    ...
    write_shops()
    write_card_price(all_records)

    _print_summary(summary, all_records)


def _print_summary(summary: list, all_records: List[Dict]) -> None:
    print("\n===== 収集サマリー =====")
    total = 0
    for s in summary:
        n = s["buy"] + s["sell"]
    ...
    prices = [p for r in all_records for p in (r["buy_price"], r["sell_price"]) if p is not None]

    print(f"\n合計取得件数: {total}")
    print(f"高額カード({HIGH_VALUE_THRESHOLD:,}円以上)件数: {len(high_value)}")
    if prices:
